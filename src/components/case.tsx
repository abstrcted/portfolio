...
export type CaseSection = {
  eyebrow?: string;
  title: string;        // Full heading shown in the content
  tocLabel?: string;    // Short label shown in the TOC
  intro?: string;
  bullets?: CaseBullet[];
  imageSrc?: string;
  imageAlt?: string;
...
export default function Case({
  ...
  return (
    <main className="container mx-auto px-0 md:px-6 py-12 md:py-16">
      {/* Top block */}
      ...
      {/* Body grid: TOC (2) | Narrative (6) | Right rail (4) */}
      <div className="grid grid-cols-12 gap-12 items-start px-6 md:px-0">
        {/* TOC */}
          ...
          {sections.map(
            (s, i) =>
              s.imageSrc && (
                <div
                  key={`rail-${i}`}
                  className="relative aspect-[4/5] rounded-2xl overflow-hidden bg-neutral-200"
                >
                  <Image
                    src={s.imageSrc}
                    alt={s.imageAlt || ""}
